...
def get_merged_nodules(root: str):
    # Merge nodules from train set

    prefix = 'train'
    fname_gtNodulesFleischner = os.path.join(root, '{}Nodules.csv'.format(prefix))
    gtNodules = readCsv(fname_gtNodulesFleischner)
    for line in gtNodules:
        print(line)
    gtNodules = joinNodules(gtNodules)
    path_out = os.path.join(root, '{}Nodules_gt.csv'.format(prefix))
